...
if (isset($_COOKIE['token']) && isset($_GET['media_num']) && isset($_GET['zip_name'])) {
    ...
    if(file_exists($downloadDir)) {
        ...
        if(file_exists($tmpDir . '/' . $_GET['zip_name']))
            echo '{"percentage": 100, "status" : true, "md5": "'. hash_file('md5', $tmpDir . '/' . $_GET['zip_name']) . '", "sha256": "' . hash_file('sha256', $tmpDir . '/' . $_GET['zip_name']) . '", "url" : "./tmp/' . $_COOKIE['token'] . '/' . $_GET['zip_name'] .'"}';
        else
